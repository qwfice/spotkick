...
# Simple privacy-respecting request counter (no cookies, no tracking, just a number)
prediction_count = 0
...
def predict_penalty(player_name: str, context: Optional[str] = "knockout"):
    ...
    return {
        "player_name": name,
        "country": country,
        "overall_conversion_rate": base_rate,
        "predicted_score_probability": predicted_prob,
        "confidence": confidence,
        "total_penalties": total_taken,
        "shootout_conversion_rate": so_rate,
        "favorite_direction": fav_dir,
        ...
        "wc_group": wc_group,
        "is_primary_taker": is_primary
    }

@app.get("/stats")
def get_stats():
    """Privacy-respecting usage stats. No cookies. No tracking. Just numbers."""
    return {
        "predictions_generated": prediction_count,
        "players_in_db": get_player_count(),
        "nations": 48,
        "message": "No users tracked. Just predictions counted."
    }
